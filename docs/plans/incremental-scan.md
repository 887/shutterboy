...
- [x] **B.3** **Hard rule: never scan twice on cold start.** The bug `50f1d6b` fixed in tonearmboy was the repository firing a scan on first DAO subscription *and* the activity firing one on `onCreate`. Audit for double-entry before B.1 lands — there should be exactly one `scanIfChanged()` call site, owned by the repository's lazy first-collect path. — audited: no UI scan triggers exist yet (the first-collect hook + Phase I.3 manual rescan UI are deferred to their own phases). The contract is locked: future call sites MUST route through `scanIfChanged()` / `forceRescan()`, never `runScan()` directly.
