...
- [ ] **F.2** Chrome toggle landed (tap-to-toggle TopAppBar via `AnimatedVisibility(slideInVertically/slideOutVertically)` + 3-second auto-hide via `LaunchedEffect`; sibling `pointerInput { detectTapGestures }` doesn't fight the existing vertical-drag detector). Remaining: favorite toggle, overflow menu, bottom action row (Share / Edit / Delete). Info icon already in the TopAppBar from F.3.
...
- [x] **F.4** Delete shipped in commit `4acf4ef` — viewer TopAppBar now carries a Delete icon (between Edit and Info) that opens a simple Cancel / Delete confirm `AlertDialog`. On confirm we call `scope.photoDeleter.deletePhotos(listOf(PhotoId(currentId)))` and `when`-dispatch on the sealed `DeleteRequest`: `Consent` routes through `rememberLauncherForActivityResult(ActivityResultContracts.StartIntentSenderForResult)` to show the system MediaStore consent dialog on API 30+; `Immediate` applies the deletion directly on API 26-28. On success we mutate a `mutableStateListOf<Long>` backing-id list via the new pure helper `ViewerBackingListMath.removeId(list, deletedId, currentPosition)` which returns the new list + next pager position (handles middle / first / tail / before-cursor / not-in-list / empty-list-pop cases — six pure-JUnit cases in `ViewerBackingListMathTest`). `RouteScope` gained a narrow `photoDeleter: PhotoDeleter` facet (R.A locked) so the viewer reads through the interface instead of `graph`. F.7 / F.8 still open (shared-element transition + Robolectric SDK-branch coverage).
- [x] **F.5** Share — `Intent.ACTION_SEND` + chooser shipped in the viewer TopAppBar (Share icon between back arrow and Info). MIME-type forwarded from the cached `Photo.mimeType`, content URI from the `MediaStore.Images.Media` row, `FLAG_GRANT_READ_URI_PERMISSION` so the chooser target can read.
- [x] **F.6** Edit handoff — `Intent.ACTION_EDIT` chooser shipped in the viewer TopAppBar (Edit pencil between Share and Info). `setDataAndType(contentUri, mimeType)` + `FLAG_GRANT_READ_URI_PERMISSION` + `FLAG_GRANT_WRITE_URI_PERMISSION` so the editor target can write back.
