...
- [ ] **E.6** **Sort-aware section keys — retroactive verification.** Audit every `items(... key = { ... })` and every `stickyHeader(... key = ...)` in `ui/photos/`, `ui/collections/`, `ui/photos/grid/PhotosGrid.kt`, and the year-scrubber. Every key MUST include the active `(sortAxis, sortDirection)` in its identity tuple — `key = { photo -> "${sort.axis}:${sort.direction}:${photo.id}" }` shape, NOT just `photo.id`. Reference bug: tonearmboy `d75b542` ("fix LazyGrid duplicate-key crash") — when sort axis changes, two photos can land in the same section under the *previous* sort while their new section is still being composed; the duplicate key throws. Sticky banner section keys (`MAY 2021` etc.) MUST also include the sort axis, since `name_asc` reorganizes sections away from date-based bands entirely.
