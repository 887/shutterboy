...
- [ ] **A.5** Settings catalogue + `Setting<T>` abstraction — first cut. Mirror tonearmboy's M3 Expressive grouped-cards-with-pill-search shape: Look and Feel (theme, dynamic color, grid density default, thumbnail quality), Library (Manage sources, Rescan photos, Clear cache), Photos (date-header style, year-scrubber on/off, default sort), Albums (default sort, hide empty folders), Viewer (chrome auto-hide delay, double-tap zoom level), About (build, license, GitHub, easter egg). Catalogued at `ui/settings/catalog/SettingsCatalog.kt` per tonearmboy's single-source-of-truth pattern. **`Setting<T>(key, default, encode, decode)` + `EnumSetting<E>` value types defined in `data/settings/` *before the first preference key lands* — no hand-rolled `stringPreferencesKey + Flow + setter + snapshot field` quartets, ever** (R.B locked). **Facet interfaces from day one** — `ThemeSettings`, `LibrarySettings`, `PhotosSettings`, `AlbumsSettings`, `ViewerSettings`. Sub-pages take only their facet, never the wholesale repo. **`SettingsSnapshot` is forbidden.** Every catalog row's label / subtitle / keywords carry `@StringRes Int` resource ids, not inline strings — i18n-native from row one. Each row carries its own `@Composable Render(entry)` (no central `RowKind` `when` chain, R.F.13). Rows co-located per section (`ui/settings/sections/PhotosSection.kt` etc.); one aggregator `flatten`s (R.F.14, no central edit-magnet).
...
- [ ] **B.5** **Eight narrow facet interfaces, not one fat repo** (R.A locked, mirroring tonearmboy's audit lesson). `RoomGalleryRepository` implements all eight; `AppGraph` exposes each separately so consumers depend only on what they read.
...
- [ ] **C.1** Bottom navigation scaffold + `RouteScope` pattern locked from the first route. Three destinations: Photos (`Icons.Outlined.Image` / filled when active), Collections (`Icons.Outlined.Collections`), Settings (`Icons.Outlined.Settings`). Single root `Scaffold` hosts the `NavigationBar`; per-destination content swaps in via `NavHost`. Top app bar carries the destination title + a search action + an overflow. **`RouteScope` interface defined now** (R.E locked) carrying `graph`, `backStack`, `snackbar`, narrow facets, command interfaces. Per-destination `Register(scope)` extensions on the sealed `Destination` interface live one file per area: `routes/PhotosRoutes.kt`, `routes/CollectionsRoutes.kt`, `routes/SettingsRoutes.kt`. **`ShutterboyApp.kt` LOC ceiling: 150** — anything beyond means a route renderer leaked back inline; extract.
- [ ] **C.2** `ui/photos/PhotosScreen.kt` is **scaffold + top-bar + dispatch only — target ~200 LOC ceiling** (R.D locked). Per-piece sub-files: `ui/photos/grid/PhotosGrid.kt` (the `LazyVerticalGrid` body), `ui/photos/grid/MonthYearBand.kt` (the Aves inline section header), `ui/photos/grid/YearScrubber.kt`, `ui/photos/grid/EmptyPhotosState.kt`. Default density: 4 columns. Each cell uses `coil3.compose.AsyncImage` against the `PhotoEntity.contentUri` with `Size(thumbnailEdgePx, thumbnailEdgePx)` request size to keep memory in check. **Inline month-year section headers — Aves pattern, scroll-with-content (NOT pinned to top).** Implemented as full-width header items via `item(span = { GridItemSpan(maxLineSpan) }) { MonthYearBand(...) }` interleaved between the per-month photo runs in the same `LazyVerticalGrid`. Bands scroll naturally with the photos beneath them; no `stickyHeader` (which `LazyVerticalGrid` lacks anyway), no overlay pin. Band format: `MAY 2026` in `headlineSmall` weight, dim against the surface. Locale-aware month name via `DateTimeFormatter.ofPattern("MMMM yyyy", Locale.getDefault())` so non-English locales render correctly when i18n lands later. **Composables take only their narrow facet** (`PhotoSource`, not `RoomGalleryRepository`).
...
- [ ] **D.2** Folder detail screen `ui/collections/FolderDetailScreen.kt` — same Photos-tab layout but filtered to `observePhotosInFolder(folderId)`. **Repository-side filtering** (R.F.12 locked) — never client-side `.filter` over the unfiltered Flow. No year-scrubber when the folder spans <90 days (don't show controls that don't pay rent).
- [ ] **D.3** Smart-album detail screen `ui/collections/SmartAlbumDetailScreen.kt` — same shape, filtered to `observeSmartAlbum(id)`.
- [ ] **D.3.5** **Extract `GallerySurface<T>` engine** (R.D locked) the moment the third surface (Photos / Folder-detail / Smart-album-detail) lands. Strategy interface: `observe(filter): Flow<List<T>>`, `sectionKey(item): SectionKey`, `comparator(sort): Comparator<T>`, `Tile(item, callbacks)`, `EmptyState()`. One renderer engine, multiple strategies. Photos / FolderDetail / SmartAlbumDetail all delegate to the engine; the per-screen file becomes the strategy + scaffolding only.
