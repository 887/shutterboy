...
- [x] **R.F.20** Replace remaining `collectAsState(` callsites in UI with `collectAsStateWithLifecycle(` for cold-start-perf B.1 parity. Survivors (2026-05-17 audit): `PhotoViewerScreen.kt:270,278,542`, `LicensesScreen.kt:71`. (Slideshow + SettingsManageSources already migrated in the same audit.) Drops background flow-collect work while screen is paused. Shipped in commit 09be745 — viewer x3 + LicensesScreen migrated.
- [x] **R.F.21** `SearchScreen` query / filter / selectedFolder state should be `rememberSaveable` so config-change (rotation) and process-death do not blow the user's typed query + active filters. Currently plain `remember { mutableStateOf("") }` + `mutableStateListOf<SearchFilter>()` at `SearchScreen.kt:121–125`. Either lift into a `SearchViewModel(SavedStateHandle)` or wrap with custom Savers. Shipped in commit 09be745 — query + activeFilters (custom Saver in SearchFilterListSaver) + savedFolderId (re-resolved against folders flow) all rememberSaveable.
- [x] **R.F.22** Viewer-page `pointerInput(photoId, dismissThresholdPx, infoOpenThresholdPx, zoomed, isVideo)` at `PhotoViewerScreen.kt:595` re-installs the vertical-drag detector every time `zoomed` flips — i.e. every pinch frame that crosses the rest threshold. Restructure so `zoomed` is read inside the suspend block rather than as a key (the `return@pointerInput` guard already early-exits cheaply), keeping the detector installation stable across the zoom session. Hotpath: the viewer is the highest-frequency recomposition surface in the app. Shipped in commit 09be745 — rememberUpdatedState refs for zoomed + isVideo; pointerInput keys reduced to (photoId, thresholds); gate read at gesture-start + drag-end.
- [ ] **R.F.23** `PhotoViewerScreen.kt` is at 699 LOC, past the 500-LOC soft heuristic and pressing the 800-LOC hard split point. Three independent reasons-to-change live in one file: (a) gesture stack (vertical-drag dismiss, pinch/zoom, double-tap, pager horizontal), (b) chrome (TopAppBar + share/edit/favorite/delete/info actions + autohide), (c) deletion flow (consent launcher + backing-list mutator + confirm dialog). Split: `PhotoViewerChrome.kt` (TopAppBar + actions), `PhotoViewerDeleteFlow.kt` (consent + confirm + applyDeletion), leave gesture/pager wiring + `PhotoPage` in `PhotoViewerScreen.kt`. Mirrors the SRP heuristic at the top of this file.
- [x] **R.F.24** Lift the activity-scoped `CoroutineScope` out of `MainActivity.onCreate` and either (a) hang it off `ShutterboyApplication` as a `processScope = CoroutineScope(SupervisorJob() + Dispatchers.Main.immediate)` exposed via `AppGraph`, or (b) cancel it in `MainActivity.onDestroy`. As shipped today, `MainActivity.kt:26` creates a fresh scope per Activity instance and never cancels it — every config-change rotation leaks a scope, and any in-flight `forceRescan()` launched from the previous activity keeps running into a torn-down view. Recommended: use the activity's existing `lifecycleScope` (auto-cancels on destroy) — equivalent to (b) and zero new infra. Shipped in commit 09be745 — MainActivity now uses lifecycleScope.
- [x] **R.F.25** Add a single-flight guard around `RoomGalleryRepository.executeScan()`. Today `runScan()` / `scanIfChanged()` / `resetAndRescan()` have no `Mutex` / `AtomicBoolean` — two near-simultaneous callers (e.g. the cold-start `LaunchedEffect(Unit)` in `ShutterboyApp.kt:48` racing with the permission-grant `onGranted` in `MainActivity.kt:40`, or two fast Reset-cache taps) can run two `executeScan()`s concurrently. The DAO-level writes are idempotent so end-state is consistent, but duplicate MediaStore + SAF + EXIF I/O and interleaved `_scanProgress` emissions hurt UX. Remedy: `private val scanMutex = Mutex()` wrapping `executeScan()`. Trigger: now (race window is real on cold-start with permission pre-granted). Shipped in commit 09be745 — scanMutex.withLock { } wraps executeScan().
- [x] **R.F.26** SAF tree URI externally revoked (user via system settings) leaves the URI string in `SafSourcesPreferences` forever, and the next scan silently walks 0 photos with no user-visible signal. Detect via `DocumentFile.fromTreeUri(uri).canRead() == false` (or `null` return), then either auto-prune from prefs + emit a snackbar OR show a "Folder access was revoked" affordance on the Manage Sources row with a Re-grant button. Shipped in commit 09be745 — SafSourceManager returns revokedUris; RoomGalleryRepository calls scanConfig.pruneRevoked; DataStoreSafSourcesPreferences atomically prunes.
- [x] **R.F.27** Add `androidResources.generateLocaleConfig = true` to `app/build.gradle.kts` so the auto-generated `locales_config.xml` lists every shipped locale. Without this, `cmd locale set-app-locales` is inconsistent (works on fresh install / first cold start, requires multiple `am force-stop` + cold-start cycles after `pm clear`). German agent + edge-case audit both flagged. One-line gradle change. Shipped in commit 09be745 — androidResources.generateLocaleConfig = true + res/resources.properties.
- [ ] **R.F.28** R8 / ProGuard rules prep. `app/build.gradle.kts` ships `isMinifyEnabled = false` for release. When a future v1.0 production cut flips this on, kotlinx.serialization / Room / Media3 / Coil 3 / kotlinx.coroutines all need keep-rules in `proguard-rules.pro`. File the rule shopping list now so the flip is one line. Each library publishes its consumer ProGuard rules; mostly we just verify those are honored + add app-specific keeps for serialized types.
- [ ] **R.F.29** Coil 3 cache budget audit. Coil defaults: memory ~25% of `Runtime.maxMemory()`, disk ~250 MB capped at ~2% of disk. For a photo gallery these may be undersized — measure real-device hit rates first emission of `SingletonImageLoader.get(context)` before tuning. Defer until evidence demands it.
- [ ] **R.F.30** Baseline Profile recording session. `:baselineprofile` module is wired (cold-start-perf F) but `app/src/main/baseline-prof.txt` doesn't exist — recording is device-bound (needs `aosp_*` AVD or a real device; the production `google_apis_playstore` AVD blocks macrobench). Schedule a recording session before the v1.0 production cut for the ~25-35% cold-start win.
- [ ] **R.F.31** `Intent.ACTION_EDIT` orphan-edit risk: viewer's Edit handoff grants WRITE perm + dispatches. If the editor app saves to a new file, or the granted permission lapses before save, we orphan the edit (no callback). Mitigation options: (a) `registerForActivityResult` on `ACTION_EDIT` so we know when the editor returns + can refresh the photo's content URI, or (b) accept the gap and document. Today no edit-back-callback exists.
