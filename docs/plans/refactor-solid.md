...
- [ ] **R.F.8** **Open right now** — delete the scaffold's vestigial `data/DataRepository.kt` + `ui/main/MainScreen.kt` + `ui/main/MainScreenViewModel.kt` + `Navigation.kt` + `NavigationKeys.kt` + the matching `*Test.kt` files when Phase B replaces them with real code. They're template lorem-ipsum from `android create empty-activity` — currently reachable from `MainActivity` but doing nothing useful. Either do a tiny clean-up commit now (replace `MainActivity` body with a placeholder Compose tree) or wait for Phase B to overwrite them naturally. Mirror of tonearmboy R.F.8.
