...
- [ ] **R.F.20** Lift the activity-scoped `CoroutineScope` out of `MainActivity.onCreate` and either (a) hang it off `ShutterboyApplication` as a `processScope = CoroutineScope(SupervisorJob() + Dispatchers.Main.immediate)` exposed via `AppGraph`, or (b) cancel it in `MainActivity.onDestroy`. As shipped today, `MainActivity.kt:26` creates a fresh scope per Activity instance and never cancels it — every config-change rotation leaks a scope, and any in-flight `forceRescan()` launched from the previous activity keeps running into a torn-down view. Trigger: next time anyone needs a second `appScope.launch { ... }` call in `MainActivity`, or first time a leak shows up in a heap dump under rotation pressure. The fix is mechanical (~5 lines) but wants a deliberate decision between (a) process-scoped (long-running rescans survive rotation, which is probably what we want) vs (b) activity-scoped (cleaner lifecycle, but a rotation mid-scan would abort it; the next `scanIfChanged()` would just resume anyway because the gate didn't get persisted, so this is also fine).
- [ ] **R.F.21** Add a single-flight guard around `RoomGalleryRepository.executeScan()`. Today `runScan()` / `scanIfChanged()` / `resetAndRescan()` have no `Mutex` / `AtomicBoolean` — two near-simultaneous callers (e.g. the cold-start `LaunchedEffect(Unit)` in `ShutterboyApp.kt:48` racing with the permission-grant `onGranted` in `MainActivity.kt:40`, or two fast Reset-cache taps) can run two `executeScan()`s concurrently. The DAO-level writes are idempotent (`upsertAll` + `replaceWithDelta`), so the end-state is consistent, but: duplicate MediaStore + SAF tree walks, duplicate EXIF I/O, and interleaved `_scanProgress` emissions that fight each other on the progress strip. Recommended remedy: a `private val scanMutex = Mutex()` wrapping `executeScan()` so a second caller suspends on the first; or a `withReentrantLock`-style "if a scan is already running, return its in-flight `Deferred<LibrarySnapshot>`" pattern if we want concurrent callers to share the result. Trigger: now (the race window is real on cold-start with permission already granted from a prior session).
