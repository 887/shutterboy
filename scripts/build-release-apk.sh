...
#
# Output: release/shutterboy-<version>-<sha7>.apk and a release/latest.apk symlink.
#
# Signing: this uses Gradle's debug keystore by default (good for personal
# sideload). For a real published release, set SHUTTERBOY_RELEASE_KEYSTORE +
# SHUTTERBOY_RELEASE_KEY_ALIAS + SHUTTERBOY_RELEASE_KEY_PASSWORD env vars and the
...
OUT_APK="${OUT_DIR}/shutterboy-${TAG}.apk"
mkdir -p "${OUT_DIR}"

# Decide release vs debug build flavour
if [ -n "${SHUTTERBOY_RELEASE_KEYSTORE:-}" ]; then
    echo "[build-release-apk] release-signed build (keystore: ${SHUTTERBOY_RELEASE_KEYSTORE})"
...
\`\`\`
adb install -r shutterboy-${TAG}.apk
\`\`\`
