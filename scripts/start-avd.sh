...
start_avd() {
    ...
    "${EMULATOR_BIN}" \
        ...
        > /tmp/shutterboy-emulator.log 2>&1 &
...
start_scrcpy() {
    ...
    scrcpy -s "${ADB_DEVICE}" --no-audio --window-title="${SCRCPY_TITLE}" \
        > /tmp/shutterboy-scrcpy.log 2>&1 &
    sleep 1
    if is_scrcpy_running; then
        echo "[start-avd] scrcpy window: ${SCRCPY_TITLE}"
    else
        echo "[start-avd] scrcpy failed to launch — check /tmp/shutterboy-scrcpy.log" >&2
    fi
}
